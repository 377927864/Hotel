...
import cn.hotel.service.impl.SkylineServiceImpl;
...
@WebServlet(urlPatterns = "/SkylineServlet")
public class SkylineServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (Integer.valueOf(a) == 1) {
            ...
            SkylineService skylineService = new SkylineServiceImpl();
            List<Hotel> list = skylineService.Skyline(service.SearchAsKeyword(search_content,user.getUserid()));


            if (list != null) {
                request.setAttribute("_sky_hotel", list);
                request.getRequestDispatcher("/skyline.jsp").forward(request, response);
            } else {
        ...
        //按地点查找
            ...
            //HotelService service = new HotelServiceImpl();
            SkylineService skylineService = new SkylineServiceImpl();
            List<Hotel> list = skylineService.Skyline(service.SearchAsPlace(thisLng, thisLat));
            if (list != null) {
                //返回到前端
                request.setAttribute("_sky_hotel", list);
                request.getRequestDispatcher("/skyline.jsp").forward(request, response);
